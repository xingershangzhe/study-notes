...
int main(void)
{
	...
	printf("This program computes the sum os the squares of integers in a range.\n"
		   ...
		   "lower limit: ");
	start = get_long();      //函数调用1 
	printf("upper limit: ");
	stop = get_long();       //函数调用2 
	while(start != 0 || stop != 0)          //当 start 和 stop 都为 0 时跳出循环 
	{
		if(bad_limits(start,stop,MIN,MAX))      //为真才执行，因此返回值为0(false),不执行 
...
long get_long(void)
{
	...
	while(scanf("%ld",&input) != 1)         //若为整数，则不参与循环 
	{
		while((ch = getchar()) != '\n')
			putchar(ch);   //处理错误输入
